...
def plot_asymptotics_on_linear():
    ...
    fig, ax = plt.subplots()
    plot_res('N', 'num_repairs', lazy_res, ax, label='lazy')
    plot_res('N', 'num_repairs', strict_res, ax, label='strict')

    fig, ax = plt.subplots()
    plot_res('N', 'num_finds', lazy_res, ax, curve_fit=lambda N: N*log2(N+1), label='lazy')
    plot_res('N', 'num_finds', strict_res, ax, curve_fit=lambda N: N**2, label='strict')
